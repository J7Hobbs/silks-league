/**
 * PlayerPicksModal — full-screen modal showing any player's picks for any week.
 * Opened by clicking a player name in the leaderboard or results screen.
 *
 * Props:
 *   userId        string | null  — target player (null = closed)
 ...
 *   seasonRank    number?        — pre-loaded season rank (from leaderboard row)
 *   onClose       fn
 */
...
function findDefaultWeekIndex(weeks) {
  const now = new Date()
  // Exception: Saturday 12pm+ → default to current week (index 0, newest)
  if (now.getDay() === 6 && now.getHours() >= 12) return 0
  // Otherwise: most recent week whose Saturday 12pm has passed
...
export default function PlayerPicksModal({
  ...
  const [picks,       setPicks]       = useState({})   // { raceId: runner+meta }
  const [scores,      setScores]      = useState({})   // { raceId: score row }
  ...
  // Reload when target userId changes
  useEffect(() => {
    if (!userId) return
    setProfile(null)
    setWeeks([])
  ...
  // ── Initial load: profile + all weeks ──────────────────────────────────────
  ...
  // ── Load picks + scores for a specific week ────────────────────────────────
  ...
  const revealed     = isWeekRevealed(selectedWeek)
  const name         = profile?.username || profile?.display_name || profile?.full_name || displayName || 'Player'
  const totalPts     = Object.values(scores).reduce((s, sc) => s + (sc?.total_points || 0), 0)
        ...
        {weeks.length > 0 && (
          <div style={m.weekNav}>
            ...
            <div style={m.weekNavLabel}>
              {weekLoading ? '…' : formatWeekLabel(selectedWeek)}
            </div>
            <button
            ...
            <>
              ...
              {hasResults && (
                <div style={m.totalRow}>
                  ...
                  <span style={m.totalPts}>{totalPts} pts</span>
                </div>
              )}
            </>
          )}
